...
interface DashboardClientProps {
  ...
  principlesData: Record<string, { avg: number; count: number }>;
}

// ── PMBOK 8 Principles aggregate summary ─────────────────────────────────────

const PRINCIPLES = [
  { key: 'holistic',       label: 'Adopt a holistic view'                            },
  { key: 'value',          label: 'Focus on value'                                   },
  { key: 'quality',        label: 'Embed quality into processes & deliverables'      },
  { key: 'accountable',    label: 'Be an accountable leader'                         },
  { key: 'sustainability', label: 'Integrate sustainability within all project areas' },
  { key: 'empowered',      label: 'Build an empowered culture'                       },
] as const;

function PrinciplesSummary({ principlesData }: { principlesData: Record<string, { avg: number; count: number }> }) {
  const assessed = Object.keys(principlesData).length;
    ...
    <Card className="border-amber-500/20 bg-amber-500/[0.02]">
      <CardContent className="pt-4 pb-3">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {PRINCIPLES.map((p) => {
            const data = principlesData[p.key];
            if (!data) return null;
            const avg = data.avg;
            return (
              <span
                ...
                className={cn(
                  ...
                  avg >= 3 ? 'bg-amber-500/10 text-amber-700 dark:text-amber-400' :
                  'bg-rose-500/10 text-rose-700 dark:text-rose-400'
                )}
              >
                {p.label.split(' ').slice(0, 2).join(' ')} · {avg.toFixed(1)}/5
              </span>
            );
          })}
        </div>
